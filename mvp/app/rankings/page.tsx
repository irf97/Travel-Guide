...
const sortModes = ["overall", "cheapest", "nightlife", "history", "international", "gender-balance", "tourism-demand", "tourism-index", "weather", "venue-density", "confidence"] as const;
...
function styleScore(city: CityIntelligence, styles: TravelStyle[], weather: Ranked["selectedWeather"]) {
  ...
  for (const style of active) {
    ...
    if (style === "sports") score += city.venues.gyms * 2.6 + city.mobility_score * 0.28 + weather.weatherComfort * 0.24;
  }
  return clamp(score / active.length);
}

function rankCity(city: CityIntelligence, input: { month: MonthName; budget: number; nights: number; styles: TravelStyle[]; wanted: FeatureKey[]; avoid: AvoidKey[] }): Ranked {
  ...
  const confidenceScore = clamp(city.demographics.confidenceScore * 0.35 + (city.tourism.confidence === "high" ? 86 : city.tourism.confidence === "medium" ? 68 : 45) * 0.3 + selectedWeather.weatherComfort * 0.15 + city.venues.densityScore * 0.2);
  const score = clamp(
    style * 0.22 +
    fit * 0.16 +
    affordability * 0.14 +
    selectedWeather.weatherComfort * 0.14 +
    city.tourism.cityTourismDemandScore * 0.12 +
    city.venues.densityScore * 0.12 +
    city.demographics.genderBalanceScore * 0.05 +
    confidenceScore * 0.05 -
    penalty * 0.55
  );
  return { city, score, cost, selectedWeather, featureFit: fit, riskPenalty: penalty, confidenceScore };
}
...
export default function RankingsPage() {
  ...
  const ranked = useMemo<Ranked[]>(() => {
    ...
    const rows = allCities
      ...
      .map((city) => rankCity(city, { month, budget, nights, styles, wanted, avoid }));
  ...
  }, [allCities, query, continent, country, month, budget, nights, styles, wanted, avoid, sortMode]);

  const selected = ranked.find((row) => row.city.id === selectedId) ?? ranked[0];

  async function save(row: Ranked) {
    const response = await fetch("/api/trips", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: `${row.city.name}, ${row.city.country}`, cityId: row.city.id, month, budgetLevel: budget <= 800 ? "Low" : budget <= 1300 ? "Medium" : "High", goal: styles.join("-") || "balanced", computedScore: row.score, notes: `Saved from /rankings stored intelligence. Sort: ${sortMode}. Cost €${row.cost}. Weather ${row.selectedWeather.weatherComfort}. Venue density ${row.city.venues.densityScore}. Tourism demand ${row.city.tourism.cityTourismDemandScore}. International pressure ${row.city.tourism.internationalTouristPressure}.` }) });
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-10 text-white">
    <section className="mx-auto max-w-7xl rounded-[2rem] border border-white/10 bg-white/[0.055] p-6 shadow-2xl shadow-sky-950/30 backdrop-blur-xl">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-xs font-black uppercase tracking-[0.22em] text-sky-200">Stored city intelligence</p><h1 className="mt-3 text-4xl font-black tracking-[-0.055em] md:text-6xl">City rankings without live pipelines</h1><p className="mt-4 max-w-3xl text-slate-300">Rankings now use stored monthly climate averages, stored tourism baselines, stored venue-density baselines, stored demographic estimates, and deterministic logic. Hotel/flight pricing remains excluded.</p></div>
    ...
    <section className="mx-auto mt-6 grid max-w-7xl gap-6 lg:grid-cols-[1.5fr_.9fr]">
      <div className="grid gap-3">
        {ranked.slice(0, 60).map((row, index) => <button key={row.city.id} onClick={()=>setSelectedId(row.city.id)} className={`rounded-[1.5rem] border p-4 text-left backdrop-blur-xl transition ${selected?.city.id===row.city.id ? "border-sky-200 bg-sky-200/10" : "border-white/10 bg-white/[0.04] hover:border-white/25"}`}>
          <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">#{index+1} · {row.city.country} · {row.city.continent}</div><h2 className="mt-1 text-2xl font-black">{row.city.name}</h2><p className="mt-1 text-sm text-slate-400">€{row.cost} · {month} · {row.selectedWeather.avgTempC}°C avg · {row.city.tourism.sourceLabel}</p></div><div className="rounded-full bg-sky-200 px-4 py-2 text-sm font-black text-slate-950">{row.score}/100</div></div>
          <div className="mt-4 grid gap-2 md:grid-cols-7"><Mini label="Weather" value={row.selectedWeather.weatherComfort}/><Mini label="Venues" value={row.city.venues.densityScore}/><Mini label="Nightlife" value={row.city.nightlife_score}/><Mini label="Gender" value={row.city.demographics.genderBalanceScore}/><Mini label="Intl" value={row.city.tourism.internationalTouristPressure}/><Mini label="Tourism" value={row.city.tourism.cityTourismDemandScore}/><Mini label="Conf." value={row.confidenceScore}/></div>
        </button>)}
      </div>
      {selected ? <aside className="sticky top-4 h-fit rounded-[2rem] border border-white/10 bg-white/[0.055] p-5 backdrop-blur-xl">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3"><Metric label="Score" value={`${selected.score}`}/><Metric label="Cost" value={`€${selected.cost}`}/><Metric label="Weather" value={`${selected.selectedWeather.weatherComfort}`}/><Metric label="Venues" value={`${selected.city.venues.densityScore}`}/></div>
        ...
        <h3 className="mt-6 text-sm font-black uppercase tracking-[0.16em] text-sky-200">Stored monthly weather</h3>
        <div className="mt-2 rounded-2xl border border-white/10 bg-slate-950/50 p-3 text-sm text-slate-300">{month}: {selected.selectedWeather.avgTempC}°C avg · {selected.selectedWeather.rainDays} rain days · comfort {selected.selectedWeather.weatherComfort}/100 · outdoor nightlife {selected.selectedWeather.outdoorNightlifeScore}/100</div>
        <h3 className="mt-6 text-sm font-black uppercase tracking-[0.16em] text-sky-200">Stored venue density</h3>
        <div className="mt-2 grid grid-cols-2 gap-2 text-sm text-slate-300"><Metric label="Bars" value={`${selected.city.venues.bars}`}/><Metric label="Clubs" value={`${selected.city.venues.clubs}`}/><Metric label="Restaurants" value={`${selected.city.venues.restaurants}`}/><Metric label="Cafes" value={`${selected.city.venues.cafes}`}/></div>
        <h3 className="mt-6 text-sm font-black uppercase tracking-[0.16em] text-sky-200">Stored tourism baseline</h3>
        <div className="mt-2 rounded-2xl border border-white/10 bg-slate-950/50 p-3 text-sm text-slate-300">Country baseline: {selected.city.tourism.tourismNightsIndex}/100<br/>International guests: {selected.city.tourism.internationalGuestShare}% · Domestic guests: {selected.city.tourism.domesticGuestShare}%<br/>Source: {selected.city.tourism.sourceLabel}<br/>Release: {selected.city.tourism.release} · confidence {selected.city.tourism.confidence}</div>
        ...
        <p className="mt-4 text-xs leading-5 text-slate-500">Sources: {selected.city.sourceConfidence.weather}; {selected.city.sourceConfidence.tourism}; {selected.city.sourceConfidence.venues}; {selected.city.sourceConfidence.demographics}.</p>
      </aside> : null}
    </section>
  </main>;
}
