    ...
    <section className="mx-auto mt-6 grid max-w-7xl gap-6 lg:grid-cols-[1.5fr_.9fr]">
      <div className="grid gap-3">
        {ranked.slice(0, 100).map((row, index) => <article key={row.city.id} className={`rounded-[1.5rem] border p-4 backdrop-blur-xl transition ${selected?.city.id===row.city.id ? "border-sky-200 bg-sky-200/10" : "border-white/10 bg-white/[0.04] hover:border-white/25"}`}>
          <div className="flex flex-wrap items-start justify-between gap-3"><button onClick={()=>setSelectedId(row.city.id)} className="text-left"><div className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">#{index+1} · {row.city.country} · {row.city.continent} · {row.passportLabel}</div><h2 className="mt-1 text-2xl font-black">{row.city.visuals.flag} {row.city.name}</h2><p className="mt-1 text-sm text-slate-400">€{row.cost} · {month} · {row.selectedWeather.avgTempC}°C avg · pulse {row.city.pulse.demandPressure}</p></button><div className="flex gap-2"><a className="rounded-full border border-white/10 px-3 py-2 text-xs font-black text-slate-300" href={`/cities/${row.city.id}`}>City page</a><div className="rounded-full bg-sky-200 px-4 py-2 text-sm font-black text-slate-950">{row.score}/100</div></div></div>
