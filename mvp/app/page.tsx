...
function TravelerIntake(props: TravelerIntakeProps) {
  ...
  return (
    <section className="py-10">
      <SectionHeader eyebrow="Preference engine" title="Traveler Preference Intake" body="Text or voice-style input becomes a validated intent object. The visible output is a visual fingerprint; raw JSON is only for debugging." />
      <div className="grid gap-5 lg:grid-cols-[1.08fr_0.92fr]">
        ...
        <Card>
          <div className="flex items-start justify-between gap-3"><div><h2 className="text-2xl font-black text-white">Intent fingerprint</h2><p className="mt-1 text-sm text-slate-400">Readable summary generated from the current structured state.</p></div><Chip tone="good">{(intent.confidence.overall * 100).toFixed(0)}%</Chip></div>
          <div className="mt-5 grid gap-3 md:grid-cols-3"><StatPill label="Group" value={`${intent.group.size}`} note={intent.group.age_range} /><StatPill label="Budget" value={`€${intent.budget.per_person_eur}`} note={`${intent.budget.sensitivity} sensitivity`} /><StatPill label="Stay" value={`${intent.dates.nights}`} note={`${intent.dates.month} · nights`} /></div>
          <div className="mt-5"><MetricBar label="Extraction confidence" score={intent.confidence.overall * 100} /></div>
          <div className="mt-5"><FieldLabel>Travel styles</FieldLabel><div className="flex flex-wrap gap-2">{intent.travel_style.map((s) => <Chip key={s} tone="good">{labelize(s)}</Chip>)}</div></div>
          <div className="mt-5"><FieldLabel>Desired features</FieldLabel><div className="flex flex-wrap gap-2">{active.length ? active.map((feature) => <Chip key={feature}>{featureLabels[feature]}</Chip>) : <Chip tone="muted">No features selected</Chip>}</div></div>
          <div className="mt-5"><FieldLabel>Avoidances</FieldLabel><div className="flex flex-wrap gap-2">{avoid.length ? avoid.map((item) => <Chip key={item} tone="warn">{labelize(item)}</Chip>) : <Chip tone="muted">No avoidances detected</Chip>}</div></div>
          <CollapsibleJson label="View raw intent data">{JSON.stringify(intent, null, 2)}</CollapsibleJson>
        </Card>
      </div>
    </section>
  );
...
function Destinations({ rankedCities }: any) {
  return (
    <section className="py-10">
      <SectionHeader eyebrow="Transparent scoring" title="Destination Intelligence" body="Scores react to the active intent. Each city card shows visual score components first, with raw score data hidden." />
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{rankedCities.map(({ city, result }: any) => <SoftCard key={city.id}><div className="flex items-start justify-between gap-3"><div><h3 className="text-xl font-black text-white">{city.name}</h3><p className="text-sm text-slate-400">{city.country} · {formatEuro(result.estimatedCost)} pp</p></div><Chip tone="good">{result.score}/100</Chip></div><p className="mt-3 text-sm leading-6 text-slate-300">{city.notes}</p><div className="mt-3 flex flex-wrap gap-2">{city.best_neighborhoods.map((n: string) => <Chip key={n}>{n}</Chip>)}</div><div className="mt-3 flex flex-wrap gap-2">{city.risk_flags.map((risk: string) => <Chip key={risk} tone="warn">{risk}</Chip>)}</div><div className="mt-5 grid gap-3"><MetricBar label="Budget fit" score={result.explanation.budget_fit} /><MetricBar label="Social density" score={result.explanation.social_density} /><MetricBar label="Nightlife" score={result.explanation.nightlife} /><MetricBar label="History" score={result.explanation.culture_history} /><MetricBar label="Food culture" score={result.explanation.food_fit} /><MetricBar label="Feature fit" score={result.explanation.preference_feature_fit} /></div><CollapsibleJson label="View raw score data">{JSON.stringify(result.explanation, null, 2)}</CollapsibleJson></SoftCard>)}</div>
    </section>
  );
}
...
function Places({ rankedPlaces, setTempPlace }: any) {
  ...
  return (
    <section className="py-10">
      <SectionHeader eyebrow="Venue intelligence" title="Place Intelligence" body="Place cards now show why they match using meters, tags, evidence, and use-case summaries before raw data." />
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">{rankedPlaces.map(({ place, result }: any) => <SoftCard key={place.id}><div className="flex items-start justify-between gap-3"><div><h3 className="text-xl font-black text-white">{place.name}</h3><p className="text-sm text-slate-400">{place.type} · {place.neighborhood}</p></div><Chip tone="good">{result.score}/100</Chip></div><p className="mt-3 text-sm leading-6 text-slate-300">{place.notes}</p><div className="mt-3 flex flex-wrap gap-2"><Chip tone={place.business_confirmed ? "good" : "muted"}>{place.business_confirmed ? "business confirmed" : "not confirmed"}</Chip><Chip>{"€".repeat(place.price_level)}</Chip>{place.good_for.map((useCase: string) => <Chip key={useCase} tone="muted">{useCase}</Chip>)}</div><div className="mt-3 flex flex-wrap gap-2">{[...place.feature_tags, ...place.ambience_tags].map((tag) => <Chip key={tag}>{labelize(tag)}</Chip>)}</div><div className="mt-5 grid gap-3"><MetricBar label="Preference similarity" score={result.explanation.preference_similarity} /><MetricBar label="Social ambience" score={result.explanation.social_ambience} /><MetricBar label="Price fit" score={result.explanation.price_fit} /><MetricBar label="Evidence confidence" score={result.explanation.evidence_confidence} /><MetricBar label="Business confirmation" score={result.explanation.business_confirmation} /></div><CollapsibleJson label="View raw match data">{JSON.stringify(result.explanation, null, 2)}</CollapsibleJson></SoftCard>)}</div>
...
function BusinessPortal() { return <RichInfo title="Business Portal / Ambience Confirmation UI" eyebrow="Business reality layer" points={["Identity, links, opening hours, price range, and languages spoken.", "Ambience profile: quiet/social/loud, local/tourist/mixed, good for groups, solo travelers, remote work, sports, pre-drinks, and after-club food.", "Feature confirmation: matcha, specialty coffee, seafood, baked goods, cocktails, sports TV, live music, terrace, halal, late-night food.", "Generated business profiles should be visual first: ambience meters, use-case badges, evidence panel, then raw JSON hidden."]} />; }
...
function CostModel() { return <section className="py-10"><SectionHeader eyebrow="Investor planning" title="Cost Model" body="Visual breakdown first; raw cost objects can stay behind debug toggles later." /><div className="grid gap-4 md:grid-cols-3"><StatPill label="Early MVP range" value="€300–€1.5k" note="monthly planning band" /><StatPill label="Largest driver" value="AI + images" note="control with caching" /><StatPill label="Rule" value="Analyze once" note="store structured tags" /></div><Card className="mt-5"><div className="grid gap-4"><MetricBar label="Hosting" score={25} /><MetricBar label="Database / storage" score={45} /><MetricBar label="Voice transcription" score={35} /><MetricBar label="Image analysis" score={70} /><MetricBar label="LLM reasoning" score={62} /><MetricBar label="Maps / place API" score={38} /></div></Card></section>; }
