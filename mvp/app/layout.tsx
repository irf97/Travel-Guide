...
const navLinks = [
  ...
  { href: "/demographics", label: "Demo", tone: "ghost" },
  { href: "/lab", label: "Lab", tone: "ghost" },
  { href: "/portal", label: "Portal", tone: "ghost" },
  { href: "/functionality", label: "Index", tone: "ghost" },
  ...
  return (
    <html lang="en" className="dark" data-theme="cosmic">
      <body>
        ...
        <nav className="fixed bottom-2 left-1/2 z-[80] flex w-[min(1180px,calc(100vw-1rem))] -translate-x-1/2 items-center justify-center gap-1 overflow-x-auto rounded-full border border-white/10 bg-slate-950/80 px-2 py-1.5 text-[11px] font-black text-white shadow-xl shadow-sky-950/30 backdrop-blur-xl">
          {navLinks.map((link) => (
            <a
              key={link.href}
