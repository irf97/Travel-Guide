...
type Ranked = { city: WorldCity; score: number; cost: number; globalRank: number; continentRank: number; featureFit: number; riskPenalty: number; seasonality: number };
...
const travelStyleOptions: Array<{ label: string; value: TravelStyle }> = [
  ...
  { label: "Digital Nomad", value: "remote-work" }
];
const featureOptions: Array<{ label: string; value: FeatureKey }> = [
  { label: "Nightlife", value: "bars" },
  { label: "Clubs", value: "clubs" },
  { label: "Food Culture", value: "local_food" },
  { label: "Historic", value: "historic" },
  { label: "Sea", value: "sea" },
  { label: "International", value: "international_crowd" },
  { label: "Young adults", value: "young_adults" },
  { label: "Coworking", value: "coworking" }
];
const avoidOptions: Array<{ label: string; value: AvoidKey }> = [
  { label: "Too Crowded", value: "too_crowded" },
  { label: "Tourist Traps", value: "tourist_traps" },
  { label: "Expensive", value: "expensive" },
  { label: "Car Dependent", value: "car_dependent" },
  { label: "Dead Nightlife", value: "dead_nightlife" }
];
...
export function WorldCommandCenter() {
  ...
  const ranked = useMemo<Ranked[]>(() => {
    ...
    const base = pool.map((city) => {
      ...
      return { city, score: result.score, cost: result.estimatedCost, featureFit: result.featureFit, riskPenalty: result.riskPenalty, seasonality: result.seasonality };
    ...
    try {
      const response = await fetch("/api/trips", {
        ...
        body: JSON.stringify({
          ...
          goal: travelStyles.join("-") || "balanced",
          computedScore: selected.score,
          notes: `Saved from /world. Filters: ${activeFilters}. Global rank #${selected.globalRank}; continent rank #${selected.continentRank}. Estimated cost €${selected.cost}. Feature fit ${selected.featureFit}. Risk penalty ${selected.riskPenalty}. Desired: ${features.join(", ")}. Avoid: ${avoid.join(", ")}.`
        })
      });
      const json = await response.json();
      const status = json?.data?.status;
      setSaveState(status === "live" ? "saved" : "fallback");
      setSaveMessage(status === "live" ? json?.data?.message ?? "Trip saved." : json?.data?.message ?? "Trip captured in fallback mode; configure DATABASE_URL to persist.");
    } catch (error) {
      setSaveState("error");
  ...
  return <main className={s.shell}>
    ...
    <section className={s.layout}>
      <aside className={`${s.panel} ${s.left}`}>
        ...
        <div style={{marginTop:10,fontSize:11,lineHeight:1.5,color:"#94a3b8"}}>Ranking now uses budget, month, nights, travel style, feature fit, avoidances, search, and continent.</div>
      </aside>
      ...
      <section className={s.stage}>
        ...
        <div className={`${s.floatStat} ${s.analyzed}`}><div className={s.statLabel}>Cities matched</div><div style={{fontSize:30,fontWeight:950}}>{visible.length}</div><div style={{fontSize:11,color:"#86efac"}}>● Controls connected</div></div>
      ...
      <aside className={`${s.panel} ${s.right}`}>
        ...
        <div className={s.rightBody}>
          ...
          <div style={{marginTop:14,display:"grid",gap:10}}><Score label="Nightlife" value={selected.city.nightlife_score}/><Score label="Social Density" value={selected.city.social_density_score}/><Score label="History & Culture" value={selected.city.history_score}/><Score label="Food Culture" value={selected.city.food_culture_score}/><Score label="Mobility" value={selected.city.mobility_score}/><Score label="Feature Fit" value={selected.featureFit}/><Score label="Seasonality" value={selected.seasonality}/></div>
