  ...
  return <main className="min-h-screen px-4 py-8 text-white">
    ...
    <section className="mx-auto mt-6 grid max-w-7xl gap-6 xl:grid-cols-[.72fr_1.18fr_.72fr]">
      <aside className="rounded-[2rem] border border-white/10 bg-white/[0.055] p-4 backdrop-blur-xl">
        ...
        <div className="mt-4 grid gap-2">{visible.slice(0, 10).map((row)=><article key={row.city.id} className={`rounded-2xl border p-3 text-left transition ${selected?.city.id === row.city.id ? "border-sky-200 bg-sky-200/10" : "border-white/10 bg-slate-950/45 hover:border-white/25"}`}><button onClick={()=>setSelectedId(row.city.id)} className="w-full text-left"><div className="flex justify-between gap-2"><span className="font-black">#{row.globalRank} {row.city.visuals.flag} {row.city.name}</span><b>{row.score}</b></div><p className="mt-1 text-xs text-slate-400">{row.passportLabel} · gender {row.genderScore} · pulse {row.city.pulse.demandPressure}</p></button><div className="mt-3 flex flex-wrap gap-2"><button onClick={()=>setSelectedId(row.city.id)} className="rounded-full bg-sky-200 px-3 py-1 text-[11px] font-black text-slate-950">Select</button><a href={`/cities/${row.city.id}`} className="rounded-full border border-white/10 px-3 py-1 text-[11px] font-black text-slate-300">City</a><a href={`/api/city-plan?id=${row.city.id}&passport=${passport}&identity=tourists&month=${month}&nights=${nights}&budget=${budget}`} className="rounded-full border border-white/10 px-3 py-1 text-[11px] font-black text-slate-300">Plan JSON</a><button onClick={()=>setExpandedPulseId(row.city.id)} className="rounded-full border border-white/10 px-3 py-1 text-[11px] font-black text-slate-300">Pulse</button></div></article>)}</div>
      </aside>
